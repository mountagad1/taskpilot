...
// Resolved per call, never at module scope — `next build` imports this
// module importable without secrets present. See clients.ts.
...
export async function createAgentCheckout(params: {
  ...
  const { data: agent } = await admin()
    ...
    .select('id, owner_id, slug, name, price_cents, currency, status')
    .eq('id', agentId)
    .single<AgentRow>()
  ...
  const hasSeller = !!agent.owner_id
